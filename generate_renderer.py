...
reminder_messages = [
    "Get up and move â˜",
    "Time to stretch ðŸ§˜",
    "Your chair is shaping your back ðŸ‘",
    "Hey star âœ¨ you're overworking!",
    "Do 10 squats",
    "Look at something green ðŸ‘€",
    "Drink water â€“ your brain loves it ðŸ’§",
    "Five minutes to rest ðŸ™Œ",
    "Loosen your back, adjust posture ðŸ§",
    "Your body is not a flowerpot â€“ stand up!",
    "Commercial break: stretch, you're the star!",
    "Your PC isn't the only thing that needs refresh ðŸ”„",
    "High five the air, then yourself âœ‹",
    "Legs not roots â€“ use them!",
    "Your chair wants a break from you ðŸª‘",
    "Stretch like a coffee commercial â˜•",
    "Move like nobody is watching ðŸ’ƒ",
    "Five minutes of dance = free dopamine ðŸŽ¶",
    "Move before your back subscribes to the desk",
    "Not a robot (yet). Show your joints who's boss ðŸ¤–",
    "Can't change the world? Change your posture ðŸ˜„",
]

inspiration_messages = [
    {"title": "Breathe and reach", "body": "Two deep breaths and a stretch flip your mood."},
    {"title": "Bodies hate stillness", "body": "Give yours two minutes of movement."},
    {"title": "Done beats perfect", "body": "Don't wait for ideal plans â€“ just stand."},
    {"title": "Movement resets the brain", "body": "A short walk can spark a new idea."},
    {"title": "Every break is a win", "body": "Small victories stack into big shifts."},
    {"title": "You are more energetic than you think", "body": "Prove it: stand, roll shoulders, smile."},
    {"title": "Motion shows gratitude", "body": "Thank your body by letting it move."},
    {"title": "New pose, new mind", "body": "Change your literal point of view."},
    {"title": "Five minutes can change the day", "body": "Walk, stretch, dance â€“ your choice."},
    {"title": "No need to run", "body": "Just don't stay still â€“ rhythm, flexibility, joy."},
    {"title": "Bodies remember care", "body": "The more you move, the easier it feels."},
    {"title": "Movement is life", "body": "Every hour of motion is one chosen for you."},
]

celebration_messages = [
    "Amazing! Your streak grows âœ¨",
    "Bravo! Your back applauds ðŸ‘",
    "Keep going â€“ you're magnetic ðŸ’«",
    "Superhero mode: body says thanks ðŸ’ª",
    "Wow! Flexibility unlocked ðŸŒ¿",
    "Discipline unlocked â€“ keep shining ðŸŒŸ",
]
...
def js_const(name, value):
    return f"const {name} = {json.dumps(value, ensure_ascii=True, indent=2)};\n\n"
...
def build():
    parts = [
        ...
        js_const("reminderMessages", reminder_messages),
        js_const("inspirationMessages", inspiration_messages),
        js_const("celebrationMessages", celebration_messages),
        js_const("bubbleThemes", bubble_themes),
    ]
    ...
    header = textwrap.dedent(
        ...
        const saveArchive = () => saveJSON(NOTES_ARCHIVE_KEY, archivedNotes.slice(0, NOTE_HISTORY_LIMIT));
        ...
        function showCongrats(message) {
          ...
          if (message) {
            ...
            showCongrats.timeoutId = setTimeout(() => {
        ...
        function triggerReminder() {
          if (awaitingCompletion) {
            ...
            saveStats();
            updateStatsUI();
            showCongrats('Mark âœ“ after you move to keep the streak.');
          }
          playBeep();
          const randomMessage = reminderMessages[Math.floor(Math.random() * reminderMessages.length)];
          showReminderBubble(randomMessage);
        ...
        function checkNoteReminders() {
          ...
          notes.forEach((note) => {
            if (note.reminderAt && now >= note.reminderAt) {
              ...
              showReminderBubble(`ðŸ“ ${title}: ${summary}`);
        ...
        function renderNotes() {
          ...
          if (!collection.length) {
            ...
            empty.textContent = showArchive ? 'Archive is empty' : 'Press â€œ+ Newâ€ to create a note';
...
```
        ...
        function updateReminderStatus(note) {
          ...
          if (note.reminderAt) {
            ...
            if (diff > 0) {
              ...
              noteReminderStatus.textContent = 'Reminder about to pop';
            }
          } else {
        ...
        function handleAttachmentFiles(files) {
          ...
          Promise.all(tasks).then(() => {
            persistNoteChange();
            renderAttachments(note);
          }).catch((error) => console.warn('Failed to load attachment', error));
        }
        ...
        noteClearAttachments?.addEventListener('click', () => {
          const note = getSelectedNote();
          if (!note) return;
          if (note.attachments?.length && confirm('Clear all attachments?')) {
            note.attachments = [];
            persistNoteChange();
            renderAttachments(note);
          }
        ...
        noteDropZone?.addEventListener('drop', (event) => {
          event.preventDefault();
          noteDropZone.classList.remove('dragover');
          if (event.dataTransfer?.files?.length) handleAttachmentFiles(event.dataTransfer.files);
        });
